...
res_df <- res_df[1:20,c('gene', 'symbol', 'log2FoldChange', 'pvalue', 'padj')]
colnames(res_df) <- c('ENSEMBL ID', 'Symbol', 'log2FoldChange', 'p value', 'p value (adj)')
write.csv(res_df, 'Reports/rnaseq/tables_paper/top20DEgenes_indvsagg.csv', row.names = FALSE)
```
...
### Hallmark Pathways
```{r}
allograft_rej <- strsplit(fgsea_res$res_hm$leadingEdge[10], '\\s+')[[1]]
```
...
res_df <- res_df[1:20,c('gene', 'symbol', 'log2FoldChange', 'pvalue', 'padj')]
colnames(res_df) <- c('ENSEMBL ID', 'Symbol', 'log2FoldChange', 'p value', 'p value (adj)')
write.csv(res_df, 'Reports/rnaseq/tables_paper/top20DEgenes_indvsint.csv', row.names = FALSE)
```
...
res_df <- res_df[1:20,c('gene', 'symbol', 'log2FoldChange', 'pvalue', 'padj')]
colnames(res_df) <- c('ENSEMBL ID', 'Symbol', 'log2FoldChange', 'p value', 'p value (adj)')
write.csv(res_df, 'Reports/rnaseq/tables_paper/top20DEgenes_intvsagg.csv', row.names = FALSE)
```
...
CDE$DI_cluster <- clusters_patients$cluster
xcell_dcv_DI <- xcell_dcv
rownames(xcell_dcv_DI) <- ls_preprocessed$batch_info$pt_ID
xcell_dcv_DI <- xcell_dcv_DI[match(CDE$pt_ID, rownames(xcell_dcv_DI)),]
CDE$DI_cluster <- as.factor(CDE$DI_cluster)

xcell_dcv_DI['Group'] <- as.factor(CDE$DI_cluster)
xcell_dcv_DI['pt_ID'] <- CDE$pt_ID
xcell_dcv_DI <- reshape2::melt(xcell_dcv_DI, id.vars = c("pt_ID", "Group"))
colnames(xcell_dcv_DI)[4] <- 'Fraction'

ggplot(xcell_dcv_DI, aes(x=Group, y=Fraction, color = Group)) +
  geom_boxplot() +
  #ylim(0,1)+
...
```{r}
allo_i <- which(ls_preprocessed$raw_counts$Feature_gene_name %in% allograft_rej)
allo_ids <- as.character(ls_preprocessed$raw_counts$Feature[allo_i])
allo_gsym <- as.character(ls_preprocessed$raw_counts$Feature_gene_name[allo_i])
```
...
## Heatmap of Allograft rejection gene set
```{r  , fig.width = 10, fig.height = 15}
# non-directed heatmap + clinical vars
mat <- as.matrix(ls_preprocessed$vsd_mat)
mat <- mat[allo_ids,]
rownames(mat) <- allo_gsym
...
ha = HeatmapAnnotation(
  Group = ls_preprocessed$CDE$n_op2,
  SILA = ls_preprocessed$CDE$SILA,
  Stage = ls_preprocessed$CDE$Stages_simplified,
  Smoking_Status = as.factor(ls_preprocessed$CDE$Smoking_Status),
  Hist_predominant = as.factor(ls_preprocessed$CDE$Hist_predominant),
  Path_Nodule_Size_cm = ls_preprocessed$CDE$Path_Nodule_Size_cm,
  simple_anno_size = unit(0.5, "cm")
)
Heatmap(t(scale(t(mat))), name = "mat", 
        heatmap_legend_param = list(color_bar = "continuous"), 
        row_names_gp = gpar(fontsize = 8),
        column_split = ls_preprocessed$CDE$n_op2,
        #column_km = 2,
        column_names_gp = gpar(fontsize = 8), top_annotation = ha)
